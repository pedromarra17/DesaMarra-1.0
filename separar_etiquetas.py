...
def show_logo_center(width_px: int = 420):
    ...
    if logo_path.exists():
        ...
        st.markdown(f"<div style='text-align:center'><img src='data:image/png;base64,{b64}' style='width:{width_px}px;margin:0 auto;display:block'/></div>", unsafe_allow_html=True)

show_logo_center()
st.markdown("<h1 style='text-align:center;margin:.4rem 0 0 0'>Etiquetas (4 → 1) + Lista (QNT + SKU)</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center'>Para cada etiqueta: casa por <b>Pedido</b>; se não achar, pareia por <b>ordem</b>. Rodapé mostra só <b>QNT x SKU</b>.</p>", unsafe_allow_html=True)
st.divider()
...
# =============== Pedido (ID) ===============
ORDER_WORD_NEAR = re.compile(r"PEDIDO[:\s#-]*([A-Z0-9]{8,24})", re.I)
ORDER_TOKEN_RE  = re.compile(r"\b([A-Z0-9]{10,24})\b")

def extract_order_any(text: str) -> str:
    ...
    if m:
        ...
        if not tok.startswith("BR") and re.search(r"[A-Z]", tok) and re.search(r"\d", tok):
            return tok
    # fallback: primeiro token plausível
...
STOP_WORDS = ["checklist de carregamento","corte aqui","pagamento","assinatura"]

# SKU deve ter letras E números (evita códigos da etiqueta, ex.: LGO-92)
...
def extract_rows_numbered(text: str) -> list[str]:
    t = norm_heavy(text)
    # corta tudo após checklist/corte
...
def extract_products_from_quad(text: str) -> list[str]:
    """
    Extrai itens da lista: retorna somente 'QNTx SKU'.
    Conservador: ignora quadrantes com palavras típicas de ETIQUETA.
    """
    raw = norm_heavy(text)
    low = raw.lower()
    ...
    # precisa ter pelo menos 1 SKU real no bloco
...
def process_pdf(pdf_bytes: bytes, show_diag=False):
    ...
    # Coleta listas (com ou sem pedido) e TODAS as etiquetas
    pick_by_order = {}        # pedido -> [linhas]
    pick_unassigned = []      # listas sem pedido (ordem)
    labels = []               # [{'page_idx','box','text'}]
    ...
    for i in range(len(doc)):
        ...
        for rect_fitz, box_pdf in zip(qf, qp):
            ...
            if prods:
                order = extract_order_any(txt)
                if order:
                    acc = pick_by_order.setdefault(order, [])
                    # mantém a ordem e evita duplicar
                    for it in prods:
                        if it not in acc: acc.append(it)
                    if show_diag: diag_picks.append((order, acc[:3]))
                else:
                    pick_unassigned.append(prods)
                    if show_diag: diag_picks.append(("(sem pedido)", prods[:3]))
                continue  # não é etiqueta

            # etiqueta (descarta branco)
            if REMOVE_BLANK and quad_is_blank_by_raster(doc, i, rect_fitz):
                continue
            labels.append({
                "page_idx": i, "pypdf_box": box_pdf, "text": txt
            })

    # Agora GERA 1 PÁGINA POR ETIQUETA
    if not labels:
        ...
        return out.getvalue(), {"picks":diag_picks,"labels":[], "orders":[]}

    # 1) recorta todas as etiquetas em ordem de leitura
    w = PdfWriter()
    for lab in labels:
        p_src = reader.pages[lab["page_idx"]]
        x0,y0,x1,y1 = lab["pypdf_box"]
        p = deepcopy(p_src); rect = RectangleObject([x0,y0,x1,y1])
        p.cropbox = rect; p.mediabox = rect
        w.add_page(p)
    buf = io.BytesIO(); w.write(buf); buf.seek(0)
    cropped = fitz.open(stream=buf.getvalue(), filetype="pdf")

    # 2) monta páginas finais com a faixa abaixo
    final_doc = fitz.open()
    used_orders = set()
    pick_un_idx = 0
    ...
    for idx, lab in enumerate(labels):
        ...
        # tenta casar por pedido desta etiqueta
        order = extract_order_any(lab["text"])
        if order in pick_by_order and order not in used_orders:
            products = pick_by_order[order][:MAX_LINES]
            used_orders.add(order)
        else:
            # pega próxima lista disponível por ordem
            products = pick_unassigned[pick_un_idx][:MAX_LINES] if pick_un_idx < len(pick_unassigned) else []
            if pick_un_idx < len(pick_unassigned): pick_un_idx += 1
    ...
    return out.getvalue(), {"picks":diag_picks,"labels":[extract_order_any(l['text']) for l in labels]}
...
if uploaded:
    try:
        ...
        st.success("Pronto! 1 etiqueta por página, com a lista (QNT x SKU) no rodapé.")
